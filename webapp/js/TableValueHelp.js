sap.ui.define([ 
	...
	return {        
        ...
        handleFormValueHelp: async function (oEvent, oThis) {
            ...
            if (!me._tableValueHelpDialog) {
                me._tableValueHelpDialog = sap.ui.xmlfragment(
                    "zuiprdconsump.view.fragments.valuehelp.TableValueHelpDialog",
                    me
                );
        ...
        getResources: async function(pSource, pThis) {
            ...
            if (pThis._inputField == "DESTHUID") {
                var sPath = oSource.getParent().oBindingContexts[sModel].sPath;
                var oHu = pThis.getView().getModel("hu").getProperty(sPath);
                var oHdr = pThis.getView().getModel("hdr").getData().results[0];
                sEntity = "/InfoHUDestSet";
                  
                var sFilter = "HUTYPE eq '" + oHu.HUTYPE + "' and PLANTCD eq '" + oHdr.ISSPLANT + "' and SLOC eq '" + oHu.SLOC + 
                    ...
                    promiseResult = new Promise((resolve, reject) => {
                        oModel.read(sEntity, {
                            ...
                            success: function (data, response) {
                                ...
                                pThis.getView().setModel(new JSONModel(data.results), "desthuid");
                                resolve();
                            },
                            error: function (err) { 
